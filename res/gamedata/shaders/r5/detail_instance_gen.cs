...
void main(uint3 group_id : SV_GroupID, uint3 thread_id : SV_GroupThreadID)
{
    ...
    for (uint i = thread_id.x; i < total_grid_points; i += 64)
    {
        ...
        else
        {
            ...
            // Match legacy DetailManager placement: authored size variation,
            // not every plant at maximum size and facing the same direction.
            float scale = pcg_randF(r_scale, mdl.minScale * 0.5, mdl.maxScale * 0.9)
