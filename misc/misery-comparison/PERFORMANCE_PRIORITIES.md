...
1. Focus next on A-life and the shared game-update worker sequence. The source inspection below found serial callback execution, disabled AI-vision offloading despite its enabled setting, and a concrete units error in A-life's time-budget calculation. Measure the affected updates and verify simulation continuity before changing budgets or distributing shared-state work. Use the existing profiler/trace infrastructure; inspect actual object counts and capacity pressure before widening formats.
...
Concrete next A-life candidate: `set_process_time` currently computes `microseconds - microseconds * factor / 1,000,000`, then passes that value unchanged to a timer measured in seconds. At the installed values 10,000 and 0.75, single-precision evaluation gives about **9,999.992 seconds**; converting the complete remaining budget gives **0.0025 seconds**. This defeats the apparent intended switching time budget. The iterator does restart its timer on every update; first-load/precache bypass behavior also exists. No runtime correction or benefit is claimed yet. Verify configured budget, processed counts, backlog/cycle progress and switching/save behavior when testing a correction.
