...
1. The A-life time-budget units correction is now built and tested below. Next identify the recurring 41-45 ms main-thread FrameMove stall seen in both BZ/CA; neither slow frame waited for the shared worker. Use existing profiler/trace infrastructure to attribute the actual callback before distributing shared-state work. Preserve current simulation cadence and inspect capacity pressure before widening formats.
...
A separate legacy restriction is now explained: xrSheduler.cpp:377-381 maps authored schedule_min/max=1 and A-life scale 0.5 to a 265 ms target; measured means are 268-274 ms. That cadence remains unchanged. The remaining repeated interior slow frame spends 44.784 ms (BZ) / 41.173 ms (CA) in FrameMove, with zero extra worker wait. Investigate that callback next before pursuing broad simulation concurrency. Workspace evidence: outputs/implementation-evidence/MISERY_DX12_ALIFE_BUDGET_045_SUMMARY.json, BY reload-analysis.json and BZ/CA alife-analysis.json. Packaging is the next delivery step.
